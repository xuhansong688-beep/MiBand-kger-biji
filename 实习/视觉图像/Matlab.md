...
sat_parm饱和度
wexp_parm曝光适当性
首先对于全是1的权重图逐项乘以各个函数（后面详解）的权重和计算方式，得到我们需要的综合质量分数，然后对于这个综合质量分数进行依项平均
pyr = gaussian_pyramid(zeros(r,c,3)); 构建三维的递减金字塔

高斯金字塔是用来减一半的权重，拉普拉斯是细节的递减
pyr{l} = pyr{l} + w.\*pyrI{l}; 把第i张图的细节，按权重加进去
然后使用reconstruct_laplacian_pyramid(pyr)进行图像重构
#### reconstruct_laplacian_pyramid(pyr)
导入加权后的金字塔
然后进行放大然后再加到一起
最后形成正确的图像
