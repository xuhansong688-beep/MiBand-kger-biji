...
- 你把 USB 设备插进电脑
- 主机通过 **D+ / D-** 线上的电阻，判断是低速还是全速设备
    
    → 法语：**Vitesse Low-Speed / Full-Speed**
- 主机发现设备后，开始**复位设备**
...
- 主机给设备分配一个地址，以后只喊这个号
    ...
    → 法语：**Adresse de périphérique**
- 这一步用的是 **控制传输**，必须稳、不能错
    
    → 法语：**Transfert Contrôle**
...
- 不用单根线，用 **D+ 和 D- 两根线反着来**
    
    → 法语：**Signal différentiel**
- 编码方式：**0 就翻转，1 不变**
...
- 每包开头：8 个 0 用来同步时钟
    ...
    → 法语：**Sync / SOP (Start of Packet)**
- 每包结尾：两根线同时变低，表示结束
    
    → 法语：**EOP (End of Packet)**

## 6. 总线按节奏跑（帧）

- 主机每 **1ms** 发一次同步信号，把时间切成一段一段
    
    → 法语：**Trame / Frame**
- 每个帧里，主机轮流问各个设备：你有数据吗？
